													...
													@foreach(Session::get('cart')->items as $product)
														<li class="product cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}">
															<div class="product-details">
																<div class="content">
																	<a href="{{ route('front.product',$product['item']['slug']) }}"><h4 class="product-title">@if(Session::get('language') == '1') {{mb_strlen($product['item']['name_en'],'utf-8') > 35 ? mb_substr($product['item']['name_en'],0,35,'utf-8').'...' : $product['item']['name_en']}} @else {{mb_strlen($product['item']['name_ar'],'utf-8') > 35 ? mb_substr($product['item']['name_ar'],0,35,'utf-8').'...' : $product['item']['name_ar']}} @endif</h4></a>
