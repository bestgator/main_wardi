									...
									<div class="content">
										...
										<div class="billing-address">
											...
											<div class="row">
												...
												<div class="col-lg-6">
													<select class="form-control ship_input" name="city" id="city">
														@if(Session::get('language') == '2')
															@include('includes.cities_ar')
														@else
										...
										<div class="ship-diff-addres-area d-none">
											...
											<div class="row">
												...
												<div class="col-lg-6">
													<select class="form-control ship_input" name="shipping_city" id="city">
														@if(Session::get('language') == '2')
															@include('includes.cities_ar')
														@else
															@include('includes.cities')
											...
											<div class="payment-information">
												...
												<div class="row">
													<div class="col-lg-12">
														...
														@if($gs->cod_check == 1)
														 @if($digital == 0)
															<a class="nav-link payment" data-val="" data-show="no" name="v-pills-tab3-tab" data-form="{{route('cash.submit')}}" data-href="{{ route('front.load.payment',['slug1' => 'cod','slug2' => 0]) }}" id="v-pills-tab3-tab" data-toggle="pill" href="#v-pills-tab3" role="tab" aria-controls="v-pills-tab3" aria-selected="false">
																	<div class="icon">
																			<span class="radio"></span>
																	</div>
																	<p>
...
@endif
                            ...
                            <input type="hidden" id="cashpay-cost" name="cashpay-cost" value="0">
						...
						<ul class="order-list">
							...
							<P>
								@if($gs->currency_format == 0)
									<b id="discount">{{ $curr->name }}{{ Session::get('coupon') }}</b>
								@else 
									<b id="discount">{{ Session::get('coupon') }}{{ $curr->name }}</b>
								@endif
							</P>
							</li>
		              ...
		              <p>
						...
						<div class="packeging-area">
							...
							@foreach($shipping_data as $data)
								<input type="hidden" name="shipCost1" id="shipCost1" value="{{$data->price }}">
								<input type="hidden" name="shipCost2" id="shipCost2" value="{{$data->price_out }}">
								<div class="radio-design">
										<input type="radio" class="shipping" id="free-shepping{{ $data->id }}" name="shipping" value="{{ round($data->getRealShippingPrice() * $curr->value,2) }}" {{ ($loop->first) ? 'checked' : '' }}> 
										<span class="checkmark"></span>
										<input type="hidden" name="shipcosttext" id="shipcosttext" value="@if(Session::get('language') == '2') {{ $data->title_ar }} @else {{ $data->title }} @endif" >
										<label for="free-shepping{{ $data->id }}" id="shipLabel"> 
												@if(Session::get('language') == '1') {{ $data->title }} @else {{ $data->title_ar }} @endif

												@if($data->getRealShippingPrice() != 0)
												+ @if(Session::get('language') == '1') {{ $curr->sign }} @else {{ $curr->name }} @endif{{ round($data->getRealShippingPrice() * $curr->value,2) }}
												@endif
						...
						<div class="radio-design"  id="cashpayfees" style="display: none;">
							<input type="radio" checked>
							<span class="checkmark"></span>
							<label>
								@if(Session::get('language') == '1')  COD Fees @else رسوم الدفع عند الإستلام   @endif +
@if($gs->currency_format == 0)
	@if(Session::get('language') == '1') {{ $curr->sign }} @else {{ $curr->name }} @endif 17.00
@else 
	@if(Session::get('language') == '1') {{ $curr->sign }} @else {{ $curr->name }} @endif 17.00 {{ $curr->sign }}
@endif
						...
						@if(Session::has('coupon_total'))
							@if($gs->currency_format == 0)
								<span id="final-cost">@if(Session::get('language') == '1') {{ $curr->sign }} @else {{ $curr->name }} @endif {{ $totalPrice }}</span>
							@else 
								<span id="final-cost">@if(Session::get('language') == '1') {{ $curr->sign }} @else {{ $curr-name }} @endif {{ $curr->sign }}</span>
							@endif
...
ftotal = parseFloat(ftotal);
      if(ftotal % 1 != 0)
      {
		...
		if(pos == 0){
			$('#final-cost').html('{{ $curr->name }}'+ftotal)
		}
		else{
			$('#final-cost').html(ftotal+'{{ $curr->name }}')
		}
...
ttotal = parseFloat(ttotal);
      if(ttotal % 1 != 0)
      {
		...
		if(pos == 0){
			$('#final-cost').html('{{ $curr->name }}'+ttotal);
		}
		else{
			$('#final-cost').html(ttotal+'{{ $curr->name }}');
		}
            ...
            $.ajax({
                    ...
                    success:function(data){
                        ...
                        else
                        {
							...
							if(pos == 0){
								$('#total-cost').html('{{ $curr->name }}'+data[0]);
								$('#discount').html('{{ $curr->name }}'+data[2]);
							}
							else{
								$('#total-cost').html(data[0]+'{{ $curr->name }}');
								$('#discount').html(data[2]+'{{ $curr->name }}');
							}
...
// Step 2 btn DONE
	...
	$('#step3-btn').on('click',function(){
		...
		if($("#city").val() == 'Jeddah'){
			if(shipCost != $("#shipCost1").val()){
				var mship = $("#shipCost1").val() ;
					$('#shipping-cost').val($("#shipCost1").val());
					$('.shipping').val($("#shipCost1").val());
					$('#shipLabel').html($("#shipcosttext").val() + " " +'{{ $curr->name }}'+ $("#shipCost1").val()) ;
					var ttotal = parseFloat('{{ $totalPrice}}') + parseFloat(mship) ;
					ttotal = parseFloat(ttotal);
				      	if(ttotal % 1 != 0)
				      	{
				        	ttotal = ttotal.toFixed(2);
			...
			if(shipCost != $("#shipCost2").val()){
				var mship = $("#shipCost2").val()  ;

					$('#shipping-cost').val($("#shipCost2").val());
					$('.shipping').val($("#shipCost2").val());
					$('#shipLabel').html($("#shipcosttext").val() + " + " +'{{ $curr->name }}'+ $("#shipCost2").val()) ;
					var ttotal = parseFloat('{{ $totalPrice}}') + parseFloat(mship) ;
					ttotal = parseFloat(ttotal);
				      	if(ttotal % 1 != 0)
				      	{
	...
	$('.payment').on('click',function(){
      ...
      $('.submit-loader').show();
		...
		if(show != 'no') {
			$('.pay-area').removeClass('d-none');
			document.getElementById("cashpayfees").style.display = 'none' ;
					var mcashp = 0 ;
					$('#cashpay-cost').val(mcashp);
                var mship = $('#shipping-cost').val();
				var ttotal = parseFloat($('#tgrandtotal').val()) + parseFloat(mship) + parseFloat(mcashp);
					ttotal = parseFloat(ttotal);
					    if(ttotal % 1 != 0)
					    {
					        ttotal = ttotal.toFixed(2);
			...
			$('.pay-area').addClass('d-none');
			//if($("input[name='v-pills-tab3-tab']:checked").val()){ 
					document.getElementById("cashpayfees").style.display = '' ;
					var mcashp = 17 ;
					$('#cashpay-cost').val(mcashp);
					var mship = $('#shipping-cost').val();
				var ttotal = parseFloat($('#tgrandtotal').val()) + parseFloat(mship) + parseFloat(mpack)+ parseFloat(mcashp);
					ttotal = parseFloat(ttotal);
					    if(ttotal % 1 != 0)
					    {
					        ttotal = ttotal.toFixed(2);
