...
@section('content')
    ...
    <section class="contact-us">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="contact-section-title" style="">
                        <h1 class="text-center" style="font: normal normal bold 35px/50px Tajawal;">
                            @if(Session::get('language') == '1' ) Noura Global Branches @else فروع صيدليات نورا العالميه   @endif</h1>
                    </div>
                    <!--<form class="form-new">-->
                    <!--    <input type="text" id="addressInput" class="inp-search-new px-3" -->
                    <!--        name="addressInput" required="" value="{{ request()->get('addressInput') }}"-->
                    <!--        autocomplete="off" style="" placeholder="@if(Session::get('language') == '2' ) ادخل موقع او مكان  @else Enter Place or  Location  @endif">-->
                    <!--    <input type="hidden" id="loc_lat" />-->
                    <!--    <input type="hidden" id="loc_long" />-->
                    <!--    <button type="submit" id="searchButton" class="btn-search-inp" style="">Search</button>-->
                    <!--</form>-->
                </div>
            </div>
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-6">
                    <div class="container" id="maps-locator-div">
                        <div class="row  justify-content-center">
                            ...
                            <div class="col-12 col-lg-3 storelocator-panel px-0" id="panel">

                                <ul class="store-list">
                                    @foreach ($branches as $branch)
                                        <li class="store" id="{{ $branch->id }}"
                                            onclick="searchLoc('{{ $branch->address_latitude }}','{{ $branch->address_longitude }}','{{ $branch->nameLang() }}','{{ $branch->cityLang() }}');">

                                            <input type="hidden" class="hiden" name="address_latitude" id="address_latitude"
                                                value="{{ $branch->address_latitude }}">
                                            <input type="hidden" class="hiden" name="address_longitude"
                                                id="address_longitude" value="{{ $branch->address_longitude }}">

                                            <div id="locationSelect" name="locationSelect">
                                                <div class="title_name">
                                                    <h5 class="text-right font-main">{{ $branch->nameLang() }}</h5>
                                                </div>
                                                <div class="address text-right font-main">
                                                    <p><i class="fas fa-map-marker-alt"></i> {{ $branch->cityLang() }}
                                                    </p>
                            ...
                            <div class="col-12 col-lg-8" id="map" ></div>
...
<script type="text/javascript"
    src="//maps.googleapis.com/maps/api/js?key=AIzaSyCIyqaj_vYHBGogXIqMBsmqw6rs6Z3u8Yc&callback=initMap&libraries=places&language=en"
    async defer></script>
...
<script type="text/javascript">
    ...
    function initMap() {
        ...
        var sydney = {
            lat: 22.827453215001576,
            lng: 40.85511422293166
        };
        ...
        for (i = 0; i < LocaArr.length; i++) {
            ...
            var latlng = {
                lat: parseFloat(LocaArr[i].address_latitude),
                lng: parseFloat(LocaArr[i].address_longitude)
            };
