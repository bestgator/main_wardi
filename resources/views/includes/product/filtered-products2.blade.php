					...
					@foreach ($prods as $key => $prod)
									<div class="col-lg-3 col-md-4 col-6 remove-padding">
										...
										<a href="{{ route('front.product', $prod->slug) }}" class="item">
										    <div style="top: 1067px;left: 990px;width: 100%;background: #464646 0% 0% no-repeat padding-box;border-radius: 10px;opacity: 1;">
										    <h4 style="top: 1090px;left: 1004px;text-align: center;font: normal normal bold 15px/22px Tajawal;letter-spacing: 0px;color: #FFFFFF;opacity: 1;">
										        <span>@if(Session::get('language') == '1') {{ $banner->discount_precentage }}% off @else وفر {{ $banner->discount_precentage }}% @endif</span>
										        </h4> 
										        </div>
											<div class="item-img">
...
<img class="img-fluid" src="@if($prod->thumbnail)  @if(file_exists('assets/images/thumbnails/'.$prod->thumbnail)) {{   asset('assets/images/thumbnails/'.$prod->thumbnail) }} @else  {{ asset('assets/images/noimage.png') }} @endif @else  {{ asset('assets/images/noimage.png') }} @endif" alt="">
											</div>
											<div class="info">
												<div class="stars">
                            <div class="ratings">
                                ...
                                <div class="full-stars" style="width:{{App\Models\Rating::ratings($prod->id)}}%"></div>
												...
												<h4 class="price" style="font-family: 'Tajawal';">{{ $prod->showPrice() }} <br /><del><small>{{ $prod->showPreviousPrice() }}</small></del></h4>
