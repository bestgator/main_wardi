    ...
    <div class="row">
        ...
        <h4 class="heading"><a class="add-btn" href="{{ url()->previous() }}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a></h4>
                <ul class="links">
                    ...
                    <li>
                        <a href="{{ route('admin-message-index') }}">{{ __('Prescription Request') }}</a>
                    </li>
                </ul>
        @endif
        </div>
...
<div class="order-table-wrap support-ticket-wrapper ">
        ...
        @if($conv->user_id != null)
             ...
             @else
             @if($conv->photo != null)
             <img  src=" {{ asset('assets/images/preception/'.$conv->photo) }} " alt="">
             @else 
             <img  src="{{ asset('assets/images/noimage.png') }} " alt="">@endif
        @endif
                    </div>
