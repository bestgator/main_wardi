...
class Product extends Model
{
    ...
    protected $fillable = ['user_id','category_id','product_type','affiliate_link','sku', 'subcategory_id', 'childcategory_id', 'attributes', 'name_en','name_ar', 'photo', 'size','size_qty','size_price', 'color', 'details_en','details_ar','price','previous_price','stock','policy_en','policy_ar','status', 'views','tags','featured','best','top','hot','latest','big','trending','sale','features','colors','product_condition','ship','meta_tag','meta_description','youtube','type','file','license','license_qty','link','platform','region','licence_type','measure','discount_date','is_discount','whole_sell_qty','whole_sell_discount','catalog_id','slug'];
    ...
    public  function setCurrency() {
        ...
        if($gs->currency_format == 0){
            return $curr->nameLang().$price;
        }
        else{
            return $price.$curr->nameLang();
        }
    }
    ...
    public function showName() {
        $lang = Session::get('language') ;
        $realname = $this->name_ar ;
        if($lang == '1'){
            $realname = $this->name_en ;
        }elseif($lang == '2'){
            $realname = $this->name_ar ;
        }
        ...
        $name = mb_strlen($realname ,'utf-8') > 55 ? mb_substr($realname ,0,55,'utf-8').'...' : $realname ;
        return $realname ;
    }
