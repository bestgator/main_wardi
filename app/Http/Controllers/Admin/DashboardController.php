...
class DashboardController extends Controller
{
    ...
    public function MoveBrand(Request $request){
        $contacts = explode(',', $request->inputArr) ;
        $inputs['brand_id'] = $request->brand_id ;
        
        foreach ($contacts as $contact) {
            $inputs['product_id'] = $contact ;
            $updates = \DB::table('product_brands')->where('product_id',$contact)->first() ;
            if($updates){
                \DB::table('product_brands')->where('product_id',$contact)->update(['brand_id' => $inputs['brand_id']]) ;
            }else{
                \DB::table('product_brands')->insert($inputs) ;
            }
        }
    ...
    public function ActionFilterDelete(Request $request){
        $contacts = explode(',', $request->inputArr) ;
        
        foreach ($contacts as $contact) {
            Product::where('id' , $contact)->delete() ;
        }
    ...
    public function ActionFilter(Request $request)
    {
        ...
        //$childcategory_id = $request->childcategory_id ;
            ...
            if($input['category_id'] != 0 ){
                foreach ($contacts as $contact) {
                    Product::where('id' , $contact)->update($input) ;
                }
            }
