...
class ProductController extends Controller
{
    ...
    {
         $datas = Product::where('product_type','=','normal')->orderBy('id','desc')->limit(4000)->get();
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('b_name', function(Product $data) {
                                 $stck = \DB::table('brands')->join('product_brands','product_brands.brand_id','=','brands.id')->where('product_brands.product_id',$data->id)->pluck('name_ar')->first();
                                return $stck;
                            })
                            ...
                            ->editColumn('bulk', function(Product $data) {
                                return '<td><input type="checkbox" name="bulkdelete[]" class="checkbox" value='.$data->id.'></td>';
                            })
                            ->rawColumns(['name', 'status','b_name', 'action','bulk'])
                            ->toJson(); //--- Returning Json Data To Client Side
    }
    
    
     //*** JSON Request
    public function datatables955()
    {
         $datas = DB::table('brands')->select('*');
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('action', function($datas) {
                                ...
                                return '<div class="godropdown"><button class="go-dropdown-toggle"> Actions<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="javascript:;" data-href="' . route('admin-brand-delete',$datas->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> Delete</a></div></div>';
                            })
                            ->addColumn('productss', function($datas) {
                                $buttonss = '<div class="action-list"><a href="' . route('admin-brands-products', $datas->id) . '" class="attribute" >Show Products</a>';
                                 $buttonss .= '</div>';
                                return $buttonss;
                            })
                            ->rawColumns([ 'action','productss'])
                            ->toJson(); //--- Returning Json Data To Client Side
    }
    
    public function datatables9995($id)
    {
         $datas = Product::whereIn('id',\DB::table('product_brands')->where('brand_id',$id)->pluck('product_id'))->get();
    ...
    //*** JSON Request
    public function datatables95($id)
    {
         $datas = Product::where('category_id','=',$id)->orderBy('id','desc')->get();
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('cat_name', function(Product $data) {
                                $stck = Category::find($data->category_id);
                                
                                return $stck->name_ar;
                            })
                             ->editColumn('scat_name', function(Product $data) {
                                $stck = Subcategory::find($data->subcategory_id);
                                if($stck)
                                    return $stck->name_ar;
                            })
                             ->editColumn('ccat_name', function(Product $data) {
                                $stck = Childcategory::find($data->childcategory_id);
                                
                                if($stck)
                                    return $stck->name_ar;
                            })
                            ->editColumn('b_name', function(Product $data) {
                                $stck = \DB::table('brands')->join('product_brands','product_brands.brand_id','=','brands.id')->where('product_brands.product_id',$data->id)->pluck('name_ar')->first();
                                return $stck;
                            })
                            ->editColumn('bulk', function(Product $data) {
                            ...
                            ->addColumn('action', function(Product $data) {
                                return '<div class="godropdown"><button class="go-dropdown-toggle"> Actions<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="' . route('admin-prod-edit',$data->id) . '"> <i class="fas fa-edit"></i> Edit</a></div>';
                            })
                            ->rawColumns(['name','cat_name','ccat_name','scat_name','b_name','bulk','action'])
                            ->toJson(); //--- Returning Json Data To Client Side
    }
    ...
    {
        return view('admin.product.category_products',compact('id'));
    }
    
     public function BrandsProducts($id)
    {
        return view('admin.product.brand_products',compact('id'));
    }
    ...
    {
        \DB::table('brands')->insert(['name_en' => $request->name_en,'name_ar' => $request->name_ar]) ;
    ...
    public function store(Request $request)
    {
        ...
        if($prod->type != 'Physical'){
            $prod->slug = str_slug($data->name_en,'-').'-'.strtolower(str_random(3).$data->id.str_random(3));
        }
        else {
            $prod->slug = str_slug($data->name_en,'-').'-'.strtolower($data->sku);
        }
...
if (!Product::where('sku',$line[0])->exists()){
            if (!Product::where('name_en',$line[4])->exists()){
                //--- Validation Section Ends
                ...
                $input['type'] = 'Physical';
                $input['sku'] = str_random(3).substr(time(), 6,8).str_random(3) ;

                $input['category_id'] = "";
                $input['subcategory_id'] = "";
                $input['childcategory_id'] = "";

                $mcat = Category::where(DB::raw('lower(name_en)'), strtolower($line[1]));
                ...
                if($mcat->exists()){
                    ...
                    if($line[2] != ""){
                        $scat = Subcategory::where(DB::raw('lower(name_en)'), strtolower($line[2]));
                        ...
                        if(!empty($line[2])){
                            $scat = Subcategory::create(['category_id' => $input['category_id'],'name_en' => $line[2]]) ;
                            $input['subcategory_id'] = $scat->id;
                        }
                    }
                    if($line[3] != ""){
                        $chcat = Childcategory::where(DB::raw('lower(name_en)'), strtolower($line[3]));
                        ...
                        if(!empty($line[3])){
                            $scat = Childcategory::create(['subcategory_id' => $input['subcategory_id'],'name_en' => $line[3]]) ;
                            $input['childcategory_id'] = $scat->id;
                        }
                    }
                    ...
                    if(empty($input['childcategory_id']))
                        $input['childcategory_id'] = 1;


                $curImage = explode(',', $line[6]) ;
                $input['photo'] = $curImage[0];
                $input['name_en'] = $line[4];
                $input['name_ar'] = $line[5];
                $input['details_en'] = $line[7];
                $input['details_ar'] = $line[8];
...
//                $input['childcategory_id'] = $request->childcategory_id;
                ...
                $input['stock'] = "50";//$line[11];
                $input['size'] = $line[12];
                $input['size_qty'] = $line[13];
                $input['size_price'] = $line[14];
                $input['youtube'] = $line[17];
                $input['policy_en'] = $line[18];
                $input['policy_ar'] = $line[19];
                ...
                // Conert Price According to Currency
                if(is_numeric($input['price']) && is_numeric($sign->value))
                    $input['price'] = ($input['price'] / $sign->value);
                else
                    $input['price'] = 0 ;

                if(is_numeric($input['previous_price']) && is_numeric($sign->value))
                    $input['previous_price'] = ($input['previous_price'] / $sign->value);
                else
                    $input['previous_price'] = 0 ;
                ...
                //$curImage = explode(',', $line[6]) ;
                //$img = Image::make($curImage[0])->resize(285, 285);
                //$thumbnail = time().str_random(8).'.jpg';
                //$img->save(public_path().'/assets/images/thumbnails/'.$thumbnail);
                $prod->thumbnail  = $curImage[0];
                $prod->update();
