...
class StripeController extends Controller
{
  ...
  public function __construct()
    ...
    public function store(Request $request){
        ...
        $myReq = $request->all();
        if(Session::has('cart')){
            $myCa = Session::get('cart');
            $productsGateway =[];
            $amount = 0;
            
                foreach($myCa->items as $prod)
                {
                    $product = Product::find($prod['item']['id']);
                    array_push($productsGateway , [
                        "ItemName" => "product" . $product->id ,
                        "UnitPrice" => $prod['item']['price'] ,
                        "Quantity" => $prod['qty']
                    ]);  
                    //$amount += $prod['item']['price']  * $prod['qty'];  
                            
                }
                array_push($productsGateway , [
                    "ItemName" => "Shipping Cost" ,
                    "UnitPrice" => $request->shipping_cost ,
                    "Quantity" => '1',
                ]);
                
                
                //$shipingPrice = $request->shipping_cost ;
           ...
           mail($to,$subject,$msg,$headers);
            ...
            Session::forget('coupon_percentage');
                    ...
                    $data = [
                        ...
                        "CustomerMobile"=> Auth::guard('web')->user()->phone,
                        "InvoiceValue" => $request->total ,
                        "CallBackUrl" => $success_url ,
                        "Language" => "en" ,
                        "ErrorUrl"=> "https://wardi.me/checkout",
                        "CustomerReference" => "ref".Auth::guard('web')->user()->id ,
                        "CustomerAddress"=> ['Address' => Auth::guard('web')->user()->address ],
                        "InvoiceItems" => $productsGateway ,
                    ];
                    ...
                    //   dd($request->all());
                      $n= json_encode($data);
                        ...
                        if ($json['IsSuccess'] != true) {
                            return redirect()->to("https://wardi.me/checkout") ;
                        } else {
                            ...
                            $nCard= json_encode($dataCard);
                    
                            $curl = curl_init();
                            curl_setopt_array($curl, array(
                              CURLOPT_URL => "$payment_url",
                              CURLOPT_CUSTOMREQUEST => "POST",
                              CURLOPT_POSTFIELDS => $nCard,
                              CURLOPT_HTTPHEADER => array("Authorization: Bearer $token","Content-Type: application/json"),
                            ));
                            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
                            $response = curl_exec($curl);
