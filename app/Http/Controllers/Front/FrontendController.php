...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
	...
	public function index(Request $request)
	{
        ...
        $sliders = DB::table('sliders')->where('lang',$lang)->where('status','Active')->get();
        $top_small_banners = DB::table('banners')->where('type','=','TopSmall')->where('lang',$lang)->get();
        $top_banners = DB::table('banners')->where('type','=','TobBanner')->where('lang',$lang)->get();
        $home_cat_banners = DB::table('banners')->where('type','=','HomeCatBanner')->where('lang',$lang)->get();
        $ps = DB::table('pagesettings')->find(1);
        //$feature_products =  Product::where('featured','=',1)->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(8)->get();
        $trending_products =  Product::where('status','=',1)->select($selectable)->inRandomOrder()->take(20)->get();
	    return view('front.index',compact('ps','top_banners','sliders','top_small_banners','trending_products','services','home_cat_banners'));
	}
    ...
    public function extraIndex()
    {
        ...
        $services = DB::table('services')->where('user_id','=',0)->get();
        $bottom_small_banners = DB::table('banners')->where('type','=','BottomSmall')->where('lang',$lang)->get();
        ...
        $skin_products =  Product::where('category_id','=',4)->where('status','=',1)->select($selectable)->inRandomOrder()->limit(20)->get();
        $hair_products =  Product::where('category_id','=',16)->where('status','=',1)->select($selectable)->inRandomOrder()->limit(20)->get();
        $makeup_products =  Product::where('category_id','=',2)->where('status','=',1)->select($selectable)->inRandomOrder()->limit(20)->get();
        $clicks = ProductClick::limit(20)->inRandomOrder()->pluck('product_id');
        $home_cat_banners = DB::table('banners')->where('type','=','HomeCatBanner')->where('lang',$lang)->get();
        $latest_products =  Product::whereIn('id',$clicks)->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(20)->get();

        $trending_products =  Product::where('status','=',1)->select($selectable)->inRandomOrder()->take(20)->get();
        $sale_products =  Product::where('sale','=',1)->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(9)->get();

        return view('front.extraindex',compact('ps','services','reviews','large_banners','bottom_small_banners','best_products','top_products','hot_products','latest_products','big_products','trending_products','sale_products','home_cat_banners','discount_products','partners','skin_products','makeup_products','hair_products'));
    }
...
// CURRENCY SECTION ENDS
    ...
    public function autosearch($slug)
    {
        if(mb_strlen($slug,'utf-8') > 1){
            $search = ' '.$slug;
            $prods = Product::where('status','=',1)->where('name_ar', 'like', '%' . $search . '%')->orWhere('name_en', 'like', '%' . $search . '%')->orWhere('name_en', 'like', $slug . '%')->take(10)->get()->reject(function($item){
...
// -------------------------------- CONTACT SECTION ----------------------------------------
    ...
    {
        $banner = Banner::where('id',$id)->where('type','=','BottomSmall')->first() ;
        $prods = [] ;
        if($banner->tags != ""){
            $tags = explode(',', $banner->tags) ;
             
            $prods = Product::where('status','=',1)->where(function ($query) use ($tags) {
                 for($i=0;$i<count($tags); $i++){
                    $query->orWhere('name_en', 'like', '%' . $tags[$i] . '%') ;
                 } 
            })->get();
            
            if(count($prods) > 0 ){
                if($banner->offer_change == '1'){
                    foreach ($prods as $prod) {
                        $discount_value = ($prod->price / 100) * $banner->discount_precentage;
                        $new_price = $prod->price - $discount_value;
                        Product::where('id','=',$prod->id)->update(['previous_price' => $prod->price, 'price' => $new_price]) ; 
                    }
                    $newBanner = Banner::where('tags','LIKE',$banner->tags)->get() ;
                    foreach ($newBanner as $newBan) {
                        $newBan->offer_change = '0' ;
                        $newBan->save() ;
                    }
                }
            }
        ...
        return view('front.offers',compact('prods','banner'));
    }

    public function pharmacy()
    {
        $ps =  DB::table('pagesettings')->where('id','=',1)->first();
        return view('front.pharmacy',compact('ps'));
    }

    public function receipt()
    {
        return view('front.receit');
    }
    ...
    public function pharmcyemail(Request $request)
    {
        $gs = Generalsetting::findOrFail(1);
        $ps = DB::table('pagesettings')->where('id','=',1)->first();
        $subject = "Email From Of ".$request->name;
        ...
        $phone = $request->phone;
        $from = $request->email;
        $msg = "Name: ".$name."\nEmail: ".$from."\nPhone: ".$phone."\nMessage: ".$request->pname."  ".$request->address;
        if($gs->is_smtp)
        {
        $data = [
            'to' => $to,
    ...
    public function preception(Request $request)
    {
        ...
        $gs = Generalsetting::findOrFail(1);
        $ps = DB::table('pagesettings')->where('id','=',1)->first();
        $input = $request->only('name','email','phone','address','message') ;
        if ($file = $request->file('photo'))
            {
                $name = time().$file->getClientOriginalName();
                $file->move('assets/images/preception',$name);
                $input['photo'] = $name;
            }
