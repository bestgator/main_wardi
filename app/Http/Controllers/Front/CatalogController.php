...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
      ...
      $prods = Product::when($cat, function ($query, $cat) {
                                  ...
                                  //     return $query->whereRaw('MATCH (name_en) AGAINST (? IN BOOLEAN MODE)' , array($search));
                                  // })
                                  ->when($search, function ($query, $search) {
                                      return $query->where('name_en','like' , '%'.$search.'%')->orWhere('name_ar','like' , '%'.$search.'%');
                                  })
    ...
    public function product($slug)
    {
        ...
        $vendors = Product::where('status','=',1)->whereIn('id',\DB::table('product_brands')->where('product_id',$productt->id)->pluck('product_id'))->take(8)->get();
