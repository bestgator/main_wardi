        ...
        if(Auth::guard('web')->check())
        {
                ...
                {                        
                    ...
                    if(count($users) == 1)
                    {
                        ...
                        //$shipping_data  = DB::table('shippings')->where('user_id','=',$users[0])->where('id',$ship_id)->get();
                        $shipping_data = Shipping::where('user_id','=',$users[0])->where('id',$ship_id)->get();
                        if(count($shipping_data) == 0){
                            //$shipping_data  = DB::table('shippings')->where('user_id','=',0)->where('id',$ship_id)->get();
                            $shipping_data  = Shipping::where('user_id','=',0)->where('id',$ship_id)->get();
                        }
                        else{
...
// If guest checkout is Deactivated then display pop up form with proper error message
                ...
                if($gs->multiple_shipping == 1)
                {
                    ...
                    if(count($users) == 1)
                    {
                        ...
                        $shipping_data  = Shipping::where('user_id','=',$users[0])->get();

                        if(count($shipping_data) == 0){
                            //$shipping_data  = DB::table('shippings')->where('user_id','=',0)->get();
                            $shipping_data  = Shipping::where('user_id','=',0)->get();
                        }
                        else{
                            $vendor_shipping_id = $users[0];
                        }  
